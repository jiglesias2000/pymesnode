...
let json_ArticulosFamilias = fs.readFileSync("ArticulosFamilias.json", "utf-8");
let ArticulosFamilias = JSON.parse(json_ArticulosFamilias);

let json_Articulos = fs.readFileSync("Articulos.json", "utf-8");
...
// app.get('/', (req, res) => {
//   res.send(
//     'Bienvenidos a la demo de webapi! <br> servidor/articulos <br> servidor/articulos/139'
//   );
// });

app.get("/api/articulosfamilias", async function (req, res) {

  const sql = require("mssql");
  var sqlConfig = {
    server: "servidor-pymes.database.windows.net", // Use your SQL server name
    database: "pymes", // Database to connect to
    user: "admin-pymes", // Use your username
    password: "ClaveServidor1", // Use your password
    //port: 1433,
    ...
    try {
     ...
     const result = await sql.query("select * from articulosfamilias") // where id = ${value}
     //console.dir(result)
...
app.get("/articulosfamilias", (req, res) => {
  // saving the array in a file
  res.json(ArticulosFamilias);
});
...
app.get("/articulos/:id", (req, res) => {
  ...
  let item = Articulos.filter((x) => x.IdArticulo == id)[0];
...
app.delete("/articulos/:id", (req, res) => {
  ...
  if (pos >= 0) {
    ...
    Articulos.splice(0, 1); // no persiste porque cada peticion se vuelve a crear el array.
    console.log(Articulos.length);

    // grabar datos
    const json_items = JSON.stringify(Articulos);
    fs.writeFileSync("Articulos.json", json_items);
...
// let ArticulosFamilias = [{"IdArticuloFamilia":1,"Nombre":"Accesorioss"},{"IdArticuloFamilia":2,"Nombre":"Audio"},{"IdArticuloFamilia":3,"Nombre":"Celulares"},{"IdArticuloFamilia":4,"Nombre":"Cuidado Personal"},{"IdArticuloFamilia":5,"Nombre":"Dvd"},{"IdArticuloFamilia":6,"Nombre":"Fotografia"},{"IdArticuloFamilia":7,"Nombre":"Frio-Calor"},{"IdArticuloFamilia":8,"Nombre":"Gps"},{"IdArticuloFamilia":9,"Nombre":"Informatica"},{"IdArticuloFamilia":10,"Nombre":"Led - Lcd"}] ;
